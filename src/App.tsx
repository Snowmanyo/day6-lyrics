...
/* 允許 .txt 欄位用 \n 表示多行 */
const unescapeNL = (s: string) => (s ?? "").replace(/\\r/g, "").replace(/\\n/g, "\n");
...
function TabButton({ active, children, onClick }: { active?: boolean; children: React.ReactNode; onClick?: () => void }) {
  return (
    <button onClick={onClick} className={`shrink-0 whitespace-nowrap rounded-lg px-3 py-1.5 text-sm ${active ? 'bg-black text-white' : 'hover:bg-black/5'}`}>{children}</button>
  );
}
function ToolbarButton({ children, onClick, type = 'button', className = '' }: { children: React.ReactNode; onClick?: () => void; type?: 'button'|'submit'|'reset'; className?: string }) {
  return <button type={type} onClick={onClick} className={`shrink-0 whitespace-nowrap rounded-xl px-3 py-1.5 text-sm hover:bg-black/5 active:scale-[0.99] ${className}`}>{children}</button>;
}
function Modal({ open, onClose, children, title }: { open: boolean; onClose: () => void; children: React.ReactNode; title: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[9500] flex items-center justify-center">
      ...
      <div className="relative z-[9600] w-[min(760px,92vw)] rounded-2xl bg-white p-4 shadow-2xl">
...
function DesktopSidebar({
  ...
  return (
    <aside className="hidden w-[320px] shrink-0 overflow-y-auto p-3 md:block">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`rounded-lg px-2 py-1 text-xs hover:bg-black/5 ${sortMode ? 'bg-black/5' : ''}`}
            title="切換排序模式（上下移動）"
          >
            {sortMode ? '完成' : '排序'}
          </button>
      ...
      <div className="space-y-3">
        {data.albums.map((a, albumIdx) => {
          ...
          return (
            <div key={a.id} className="overflow-hidden rounded-xl bg-white/70 p-2">
              {/* Header */}
              {!editing ? (
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-2">
                    <button
                      ...
                      className="shrink-0 rounded-md px-1.5 py-1 text-xs hover:bg-black/5"
                      aria-label={isCollapsed ? '展開' : '收合'}
                      title={isCollapsed ? '展開' : '收合'}
                    >{isCollapsed ? "▶" : "▼"}</button>

                    <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-white/60">
                  ...
                  <div className="flex items-center gap-1">
                    {sortMode ? (
                      <>
                        <button
                          className="rounded-md px-2 py-1 text-xs hover:bg-black/5 disabled:opacity-40"
                    ...
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <input
                          ...
                          className="w-full truncate rounded-md px-2 py-1 text-sm font-medium"
                        />
                        <input
                          type="date"
                          value={a.releaseDate}
                          onChange={e=>onUpdateAlbum(a.id,{releaseDate:e.target.value})}
                          className="mt-1 w-full truncate rounded-md px-2 py-1 text-xs"
                        />
                      </div>
                    </div>
                    <button
                      onClick={()=>onToggleAlbumEdit(null)}
                      className="shrink-0 rounded-lg px-2 py-1 text-xs hover:bg-black/5"
                      title="完成編輯"
                    >完成</button>
                  </div>
                  <div className="flex items-center gap-2">
                    <label className="cursor-pointer rounded-lg px-2 py-1 text-xs hover:bg-black/5">
                      上傳封面
                      <input type="file" accept="image/*" className="hidden" onChange={e=>{ const f=e.target.files?.[0]; if (f) onUploadAlbumCover(a.id, f); }} />
                    </label>
                    {a.cover && <button onClick={()=>onUpdateAlbum(a.id,{cover:""})} className="rounded-lg px-2 py-1 text-xs hover:bg-black/5">清除封面</button>}
                    <button
                      onClick={()=>{ if (confirm(`確定要刪除專輯「${a.title}」？此操作將刪除底下所有歌曲。`)) onDeleteAlbum(a.id); }}
                      className="rounded-lg px-2 py-1 text-xs text-red-600 hover:bg-black/5"
                    >刪除專輯</button>
                  </div>
                </div>
              )}
                        ...
                        <div className="grid w-full grid-cols-[1fr,auto] items-center gap-2">
                          ...
                          {sortMode ? (
                            ...
                            <button
                              ...
                              className="shrink-0 rounded-md px-2 py-1 text-xs text-red-600 hover:bg-black/5"
                              title="刪除此歌曲"
                            >×</button>
                          )}
                        </div>
...
export default function App() {
  ...
  function openImportSingle() { setModal({ type: 'import-single' }); }
  function normalizeTableHeaders(head: string[]) { return head.map(normalizeHeader); }
  function ingestSingleFile(table: string[][]) {
    ...
    setData(d => {
      ...
      for (const raw of rows) {
        ...
        const korText = idxKor >= 0 ? unescapeNL(cells[idxKor]) : "";
        const zhText  = idxZh  >= 0 ? unescapeNL(cells[idxZh])  : "";
        if (korText || zhText) {
          patch.lyrics = alignLyrics(korText, zhText);
        }

        if (idxVocab >= 0 && cells[idxVocab]) {
          const lines = unescapeNL(cells[idxVocab]).split(/\r?\n/).map(s => s.trim()).filter(Boolean);
        ...
        if (idxGrammar >= 0 && cells[idxGrammar]) {
          const lines = unescapeNL(cells[idxGrammar]).split(/\r?\n/).map(s => s.trim()).filter(Boolean);
    ...
    <div className="flex h-screen w-screen flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <a className="rounded-xl px-3 py-1.5 text-sm hover:bg-black/5" href="javascript:void(0)" onClick={()=>alert('使用說明：\n1) 側欄可排序、刪歌、編輯專輯\n2) 進入歌曲頁後，歌名下方可直接編輯「作詞/作曲」\n3) 單檔匯入：按左側「單檔匯入」，上傳 CSV/TSV（欄位含 album、song 必填；其餘選填）')}>說明</a>
        </div>
      </div>
      ...
      {modal.type === 'import-single' && (
        <Modal open title="單檔匯入（CSV/TSV）" onClose={()=>setModal({ type: null })}>
          ...
          <div className="mt-4 rounded-lg bg-white/70 p-3 text-sm text-zinc-600">
...
function SongView({ album, song, onRename, onUpdateMeta, onUpdateLyrics, onUpdateVocab, onUpdateGrammar }:{
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="min-w-0">
          {!editingTitle ? (
            <div className="flex items-center gap-2">
              ...
              <button className="rounded-md px-2 py-1 text-xs hover:bg-black/5" onClick={()=>setEditingTitle(true)} title="編輯歌名">✎</button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <input value={titleVal} onChange={e=>setTitleVal(e.target.value)} className="rounded-md px-2 py-1 text-2xl font-bold" autoFocus />
              <button className="rounded-md px-2 py-1 text-xs hover:bg-black/5" onClick={()=>{ onRename(titleVal.trim() || song.title); setEditingTitle(false); }}>儲存</button>
              <button className="rounded-md px-2 py-1 text-xs hover:bg-black/5" onClick={()=>{ setTitleVal(song.title); setEditingTitle(false); }}>取消</button>
            </div>
          )}
      ...
      {tab === "lyrics" && (
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            ...
            <textarea value={lyrKor} onChange={e=>setLyrKor(e.target.value)} rows={16} className="w-full rounded-xl p-2 font-mono text-sm" />
          </div>
          <div>
            <div className="mb-1 text-sm font-medium">歌詞（中）</div>
            <textarea value={lyrZh} onChange={e=>setLyrZh(e.target.value)} rows={16} className="w-full rounded-xl p-2 font-mono text-sm" />
          </div>
      ...
      {tab === "vocab" && <VocabEditor data={song.vocab} onChange={onUpdateVocab} />}

      {tab === "grammar" && <GrammarEditor data={song.grammar} onChange={onUpdateGrammar} />}
    </div>
  );
}
    ...
    <div className="space-y-2">
      ...
      <table className="w-full table-fixed rounded-xl">
        ...
        <tbody>
          {rows.map((r,i)=>(
            <tr key={r.id}>
              <td className="p-2"><input value={r.word} onChange={e=>setRows(x=>x.map((y,yi)=>yi===i?{...y, word:e.target.value}:y))} className="w-full rounded-md px-2 py-1" /></td>
              <td className="p-2"><input value={r.zh??""} onChange={e=>setRows(x=>x.map((y,yi)=>yi===i?{...y, zh:e.target.value}:y))} className="w-full rounded-md px-2 py-1" /></td>
              <td className="p-2 text-right"><button className="rounded-md px-2 py-1 text-xs hover:bg-black/5" onClick={()=>setRows(x=>x.filter((_,yi)=>yi!==i))}>刪除</button></td>
            </tr>
          ))}
    ...
    <div className="space-y-2">
      ...
      <table className="w-full table-fixed rounded-xl">
        ...
        <tbody>
          {rows.map((r,i)=>(
            <tr key={r.id}>
              <td className="p-2"><input value={r.pattern} onChange={e=>setRows(x=>x.map((y,yi)=>yi===i?{...y, pattern:e.target.value}:y))} className="w-full rounded-md px-2 py-1" /></td>
              <td className="p-2"><input value={r.explain??""} onChange={e=>setRows(x=>x.map((y,yi)=>yi===i?{...y, explain:e.target.value}:y))} className="w-full rounded-md px-2 py-1" /></td>
              <td className="p-2"><input value={r.example??""} onChange={e=>setRows(x=>x.map((y,yi)=>yi===i?{...y, example:e.target.value}:y))} className="w-full rounded-md px-2 py-1" /></td>
              <td className="p-2 text-right"><button className="rounded-md px-2 py-1 text-xs hover:bg-black/5" onClick={()=>setRows(x=>x.filter((_,yi)=>yi!==i))}>刪除</button></td>
            </tr>
          ))}
  ...
  return (
    <form className="space-y-3" onSubmit={e=>{ e.preventDefault(); onSubmit(title.trim(), date); }}>
      <div>
        ...
        <input value={title} onChange={e=>setTitle(e.target.value)} className="mt-1 w-full rounded-md px-2 py-1" required />
      </div>
      <div>
        <div className="text-sm">發行日</div>
        <input type="date" value={date} onChange={e=>setDate(e.target.value)} className="mt-1 w-full rounded-md px-2 py-1" />
      </div>
    ...
    <form className="space-y-3" onSubmit={e=>{ e.preventDefault(); onSubmit({ title: title.trim(), releaseDate: date, lyricist: lyricist.trim(), composer: composer.trim(), lyrics: alignLyrics(kor, zh) }); }}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <input value={lyricist} onChange={e=>setLyricist(e.target.value)} className="mt-1 w-full rounded-md px-2 py-1" placeholder="多位可用、分隔" />
        </div>
        <div>
          <div className="text-sm">作曲（選填）</div>
          <input value={composer} onChange={e=>setComposer(e.target.value)} className="mt-1 w-full rounded-md px-2 py-1" placeholder="多位可用、分隔" />
        </div>
      </div>
      <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <textarea value={kor} onChange={e=>setKor(e.target.value)} rows={8} className="mt-1 w-full rounded-xl p-2 font-mono text-sm" />
        </div>
        <div>
          <div className="text-sm">歌詞（中）選填</div>
          <textarea value={zh} onChange={e=>setZh(e.target.value)} rows={8} className="mt-1 w-full rounded-xl p-2 font-mono text-sm" />
        </div>
      </div>
      <div className="flex justify-end gap-2">
...
function SingleImport({ onTableReady }: { onTableReady: (table: string[][])=>void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <label className="cursor-pointer rounded-lg px-3 py-1.5 text-sm hover:bg-black/5">
          選擇檔案
      ...
      <textarea value={text} onChange={e=>setText(e.target.value)} rows={10} className="w-full rounded-xl p-2 font-mono text-sm" placeholder="或直接貼上 CSV/TSV 文字…" />
